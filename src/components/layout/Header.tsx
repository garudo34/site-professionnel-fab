'use client'

import { useState } from 'react'
import Link from 'next/link'
import { Menu, X } from 'lucide-react'

export default function Header() {
  const [open, setOpen] = useState(false)

  return (
    <header className="fixed top-0 right-0 left-0 z-50 border-b border-white/10 bg-neutral-950/80 backdrop-blur">
      <div className="container flex h-16 items-center justify-between">
        {/* Logo */}
        <Link href="/#top" className="text-sm font-semibold tracking-wide">
          fabien-peyres<span className="text-blue-500">.</span>fr
        </Link>

        {/* Navigation desktop */}
        <nav className="hidden items-center gap-8 text-sm text-neutral-300 md:flex">
          <Link
            href="/#realisations"
            className="relative after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all hover:after:w-full"
          >
            Réalisations
          </Link>
          <Link
            href="/#services"
            className="relative after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all hover:after:w-full"
          >
            Services
          </Link>
          <Link
            href="/#methode"
            className="relative after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all hover:after:w-full"
          >
            Méthode
          </Link>
          <Link
            href="/#about"
            className="relative after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all hover:after:w-full"
          >
            À propos
          </Link>
          <Link
            href="/#contact"
            className="relative after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-blue-500 after:transition-all hover:after:w-full"
          >
            Contact
          </Link>
        </nav>

        {/* Burger mobile */}
        <button
          onClick={() => setOpen(!open)}
          className="text-neutral-300 transition hover:text-white md:hidden"
          aria-label="Menu"
        >
          {open ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>

      {/* Menu mobile */}
      {open && (
        <div className="border-t border-white/10 bg-neutral-950/95 backdrop-blur md:hidden">
          <nav className="flex flex-col items-center gap-6 py-8 text-neutral-300">
            <Link onClick={() => setOpen(false)} href="/#realisations">
              Réalisations
            </Link>
            <Link onClick={() => setOpen(false)} href="/#services">
              Services
            </Link>
            <Link onClick={() => setOpen(false)} href="/#methode">
              Méthode
            </Link>
            <Link onClick={() => setOpen(false)} href="/#about">
              À propos
            </Link>
            <Link onClick={() => setOpen(false)} href="/#contact">
              Contact
            </Link>
          </nav>
        </div>
      )}
    </header>
  )
}
